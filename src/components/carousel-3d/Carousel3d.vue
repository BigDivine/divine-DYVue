<template>
  <div
    ...
    <controls
      ...
      :width="controlsWidth"
      :height="controlsHeight"></controls>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isNextPossible() {
      return !(!this.loop && this.isLastSlide);inverseScaling
    },
    isPrevPossible() {
  ...
  methods: {
    ...
    computeData(firstRun) {
      ...
      if (firstRun || this.currentIndex >= this.total) {
        this.currentIndex =
          parseInt(this.startIndex) > this.total - 1 ? this.total - 1 : parseInt(this.startIndex);
      }
